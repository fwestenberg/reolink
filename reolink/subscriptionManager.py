...
class manager(object):
    ...
    @property
    def renewTimer(self):
        if ( self._timeDifference is None
    ...
    async def getDigest(self):
        ...
        return {"UsernameToken": str(uuid.uuid4()),
    ...
    async def send(self, headers, data):
        ...
        try:
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(url=self._subscribe_url, data=data, headers = headers) as response:
                    response_xml = await response.text()

                    _LOGGER.debug(f"Reolink host {self._host} got response status: {response.status}. Payload: {response_xml}")
    ...
    async def subscribe(self, webhook_url):
        ...
        if self._manager_url is None or remoteTime is None or self._terminationTime is None:
            _LOGGER.error(f"Host: {self._host} failed to subscribe. Required response parameters not available.")
            return False

        self._timeDifference = await self.calcTimeDifference(localTime, remoteTime)

        _LOGGER.debug(f"Local time: {localTime:%Y-%m-%d %H:%M}, camera time: {remoteTime:%Y-%m-%d %H:%M} (difference: {self._timeDifference}), termination time: {self._terminationTime:%Y-%m-%d %H:%M:%S}")
    ...
    async def renew(self):
        ...
        if remoteTime is None:
            _LOGGER.error(f"Host: {self._host} failed to renew subscription. Required response parameters not available.")
            await self.unsubscribe()
            return False
